...
vec2 zoom_uv(vec2 uv, float _zoom, float posx, float posy, float _intensity, float _speed, float time)
{
	vec2 center = vec2(posx, posy);
	uv -= center;
	_zoom += sin(time * _speed * 6.) * _intensity * 0.5;
	uv = uv * _zoom;
	uv += center;
	return uv;
}
...
void fragment()
{
	vec2 uv = zoom_uv(UV, zoom, center_x, center_y, intensity, speed, TIME);
